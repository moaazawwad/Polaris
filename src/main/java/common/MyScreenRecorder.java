...
public class MyScreenRecorder extends ScreenRecorder {
    ...
    protected File createMovieFile(Format fileFormat) throws IOException {
        ...
        // تنظيم الملفات حسب التاريخ: recordings/yyyy-MM-dd/
        SimpleDateFormat dateFolderFormat = new SimpleDateFormat("yyyy-MM-dd");
        SimpleDateFormat dateFormat = new SimpleDateFormat("HH.mm.ss");
        Date now = new Date();
        File dateFolder = new File(movieFolder, dateFolderFormat.format(now));
        
        if (!dateFolder.exists()) {
            dateFolder.mkdirs();
        }
        
        return new File(dateFolder,
                name + "-" + dateFormat.format(now) + "." + Registry.getInstance().getExtension(fileFormat));
